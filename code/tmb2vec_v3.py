...
def completeTrajectories():
    ...
    simTrjss = cPickle.load(open('data/trajectories_200.pkl', 'rb'))
    simTrjComps = []
    for simTrjs in simTrjss:
        ...
        for i in range(0,len(simTrjs)):
            ...
            if i==0:
                ...
                dx = simTrjs[i][1] - simTrjs[i-1][1]
                dy = simTrjs[i][2] - simTrjs[i-1][2]
                rec.append(math.atan2(dy, dx))
            trjsCom.append(rec)
        simTrjComps.append(trjsCom)
    cPickle.dump(simTrjComps,open('sim_trajectories_complete','w'))
    return simTrjComps

def computeFeas():
    simTrjCompss = cPickle.load(open('sim_trajectories_complete'))
    simTrjFeas = []
    ...
    cPickle.dump(simTrjFeas, open('sim_trajectories_feas', 'w'))
    return simTrjFeas
...
def generate_normal_behavior_sequence():
    ...
    fout = open('sim_normal_behavior_sequences', 'w')
...
def vecClusterAnalysis():
    ...
    print 'Our Method'
    trVecs = []
    trs = cPickle.load(open('sim_traj_vec_normal_reverse'))
    for tr in trs:
        trVecs.append(tr[0][0])
    trVecs = np.array(trVecs)

    with open('data/trVecs_200.pkl', 'wb') as f:
    ...
    for k in k_range:
        km = KMeans(n_clusters=k, random_state=2016)
        km.fit(trVecs)
